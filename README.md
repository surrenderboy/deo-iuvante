...
```
{
    ...
    messages: string[],   // Идентификаторы сообщений в комнате
    avatarUrl: string,    // URL аватарки для комнаты, необязательно
    attachments: string[],// Отметка о прочтении
    messagesCount: number,// Полное количество сообщений в комнате
...
```
{
    ...
    read: boolean,     // Отметка о прочтении
    time: nubmer       // Timestamp времени отправки
}
``` 
...
```javascript
let rooms = await api.getUserRooms();
```
...
Параметр `message` должен быть сущностью `Message`. 
Результатом будет сущность `Message`, которая была сформирована

```javascript
let message = await api.sendMessage(message);
```

## Получить список сообщений в комнате
 
Параметр `params` должен содержать идентификатор комнаты Room._id,
может также принимать `limit` - желаемое количество сообщений, и 
`offset` - отступ с конца списка. 
Результатом будет массив сущности `Message`

```javascript
let messages = await api.getMessages(params);
```
...
Никто и не узнает, что я водитель НЛО
