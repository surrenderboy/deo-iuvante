...
1. [node lts 8](https://nodejs.org/en/)
...
```bash
# Запускает монгу при запуске проекта, данные сохраняются в data
# Если передать false, то будет использована запущенный сервер
# Запуск сервера вручную mongod --dbpath=data
export MONGO_LOCAL=true; 
# Название базы данных
export MONGO_DATABSE=yandex-shri-minsk-2018;
# Хост с базой данных
export MONGO_HOST=localhost;
# Порт быза данных. Если указано MONGO_LOCAL=true, то на этом порту будет запущена база данных
export MONGO_PORT=27017
# Хост сервера API
export SERVER_HOST=localhost;
# Порт сервера API, после изменения дев сервер вебпака не будет работать. 
# Нужно вручную исправить параметр proxy в package.json 
...
Он поднимется на порту указанном в env переменной SERVER_PORT, по умолчанию 3001

# API

Сервер api работает на [socket.io](https://socket.io/), в нем уже реализованы некоторые базовые методы.

Чтобы их использовать на клиенте подготовлен адаптер `src/api.js`. Все методы этого адаптера возвращают
[Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). 

Если у пользователя при первом входе нет сессионной куки, то для него будет автоматически создан
пользователь с фейковыми данными и в дальнейшем использован. Если хотите настоящей авторизации - реализуйте.

## Сущность User (пользователь)

Описание пользователя. Как текущего, так и любого другого.
...
```
{
    ...
    online: boolean // Есть ли пользователь в онлайне
}
``` 

## Сущность Room (комната)

Описание комнаты. Комнаты - это групповой чат. Все подписанные пользователи будут получать сообщения отправленные
в комнату

```
{
    _id: string,    // Уникальный идентификатор комнаты
    name: string,   // Имя комнаты
    users: string[] // Массив идентификаторов пользователей, которые подписаны на канал
                    // Они же User._id, смотрите выше
}
``` 

## Сущность Message (сообшение)

Сообщение отправленное пользователем в комнату
...
```
{
    ...
    created_at: nubmer // Timestamp времени отправки
}
``` 

# Сущность пагинации 

Пагинация применяется во всех списках. Если в next приходит null, то следующей страницы нет.
Не полагайтесь на параметр count, так как он может врать в некоторых случаях. Во всех запросах
к спискам можно указывать дополнительные параметры, такие как:
 
1. `limit` для ограничения числа элементов на одной странице
2. `order: {id: -1}` сортировка по полям. 1 - по возрастанию, -1 - по убыванию

```
{
    count: number,          // Число элементов в коллекции
    next: object | null,    // Параметры для загрузки следущей страницы
    items: array            // Массив с элементами
}
```

## Получить данные о текущем пользователе

В результате будет сущность User
...
Использование `await` доступно только в [`async` функциях](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function).

## Получить данные о пользователе

Получает сущность `User` по идентификатору `userId` (`User._id`)
...
В результате будет пагинация по пользователям

```javascript
let users = await api.getUsers({limit: 10});
```


## Получить список комнат

Возвращает все существующие комнаты с пагинацией.
...
Возвращает только комнаты, в которых состоит пользователь
...
Подключить текущего пользователя к комнате. 
Параметр `roomId` должен быть идентификатором комнаты Room._id. 
Результатом будет актуальная информация по комнате `Room`.

```javascript
let room = await api.currentUserJoinRoom(roomId);
```


## Присоеденить пользователя к комнате

Подключить любого пользователя к комнате. 
Параметр `userId` должен быть идентификатором пользователя User._id. 
Параметр `roomId` должен быть идентификатором комнаты Room._id. 
Результатом будет актуальная информация по комнате `Room`.

```javascript
let room = await api.userJoinRoom(userId, roomId);
```

## Удалить текущего пользователя из комнаты

Параметр `roomId` должен быть идентификатором комнаты Room._id. 
Результатом будет актуальная информация по комнате `Room`.

```javascript
let room = await api.currentUserLeaveRoom(roomId);
```

## Отправить сообщение в комнату
 
Параметр `roomId` должен быть идентификатором комнаты Room._id. 
Параметр `message` должен быть любой не пустой строкой. 
Результатом будет сущность `Message`, которая была сформирована

```javascript
let message = await api.sendMessage(roomId, message);
```

## Получить список сообщений в комнате
 
Параметр `roomId` должен быть идентификатором комнаты Room._id. 
Результатом будет пагинация по сущности `Message`

```javascript
let messages = await api.getRoomMessages(roomId);
```

## Подписка на изменение статуса пользователей

В результате придет объект следующего формата:

```
{
    status: boolean,   // Актуальный статус пользователя
    userId: string     // Идентификатор пользователя (User._id)
}
```
...
## Подписка на добавления пользователя в комнату

В результате придет объект следующего формата:

```
{
    roomId: string,    // Идентификатор комнаты (Room._id)
    userId: string     // Идентификатор пользователя (User._id)
}
```
...
```
## Подписка на удаления пользователя из комнаты
...
## Подписка на новые сообщения в комнатах

В результате придет объект сообщения (Message):
...
# Оригинальая документация по боилерплейту
